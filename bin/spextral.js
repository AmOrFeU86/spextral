...
function cmdDoctor() {
  ...
  const files = fs.readdirSync(sddDir).filter((f) => f.endsWith(".md") && f.startsWith("sdd-"));
  ...
  for (const file of files) {
    const filePath = path.join(sddDir, file);
    const content = fs.readFileSync(filePath, "utf-8");
    const { frontmatter, body } = parseFrontmatter(content);

    if (!frontmatter) {
      console.log(`  WARN  ${file} — no valid frontmatter found`);
      issues++;
      continue;
    }
    ...
    if (frontmatter.status && !VALID_STATES.includes(frontmatter.status)) {
      console.log(`  FAIL  ${file} — invalid status "${frontmatter.status}"`);
      issues++;
    }

    // Validate artifact chain
    if (frontmatter.previous_artifact && !allFiles.has(frontmatter.previous_artifact)) {
      console.log(`  FAIL  ${file} — previous_artifact "${frontmatter.previous_artifact}" not found`);
      issues++;
    }
    if (frontmatter.next_artifact && !allFiles.has(frontmatter.next_artifact)) {
      console.log(`  FAIL  ${file} — next_artifact "${frontmatter.next_artifact}" not found`);
      issues++;
    }
    ...
    if (frontmatter.fingerprint) {
      ...
      if (match) {
        ...
        if (expected !== actual) {
          console.log(`  FAIL  ${file} — fingerprint mismatch (expected ${expected}, got ${actual})`);
          issues++;
        } else {
          console.log(`  OK    ${file} — fingerprint valid (chars_${actual})`);
        }
      }
    } else {
      console.log(`  OK    ${file} — no fingerprint (skipped)`);
    }
  }
